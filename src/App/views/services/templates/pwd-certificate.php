...
$filename = generateFilename($document['fullname'], $document['date'], 'pwd certificate');

$image = imagecreatefromjpeg("assets/templates/pwd-certificate.jpg");
$color = imagecolorallocate($image, 19, 21, 22);
imagettftext($image, 40, 0, 1090, 1210, $color, $robotoBold, $document['fullname'] ?? '');
imagettftext($image, 36, 0, 700, 1460, $color, $robotoBold, $document['disability'] ?? '');
imagettftext($image, 36, 0, 820, 1950, $color, $robotoBold, getOrdinalNumber($document['date']) ?? '');
imagettftext($image, 36, 0, 1200, 1950, $color, $robotoBold, date("F", strtotime($document['date'])) ?? '');
