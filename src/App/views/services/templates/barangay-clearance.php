...
$filename = generateFilename($document['fullname'], $document['date'], 'barangay clearance');

$image = imagecreatefromjpeg("assets/templates/barangay-clearance.jpg");
$color = imagecolorallocate($image, 19, 21, 22);
imagettftext($image, 36, 0, 1390, 1220, $color, $robotoBold, $document['fullname'] ?? '');
imagettftext($image, 36, 0, 1115, 1830, $color, $robotoBold, getOrdinalNumber($document['date']) ?? '');
imagettftext($image, 36, 0, 1500, 1825, $color, $robotoBold, date("F", strtotime($document['date'])) ?? '');
imagettftext($image, 36, 0, 1430, 1960, $color, $robotoBold, $document['purpose'] ?? '');
imagettftext($image, 30, 0, 730, 2150, $color, $robotoBold, $document['fullname'] ?? '');
imagettftext($image, 30, 0, 1050, 2400, $color, $robotoBold, $document['ctcno'] ?? '');
imagettftext($image, 30, 0, 1050, 2460, $color, $robotoBold, convertToDateFormat($document['date']) ?? '');
imagettftext($image, 30, 0, 1050, 2520, $color, $robotoBold, $document['issueplace'] ?? '');
